...
2. 拖拽调整顺序、删掉不要的、给中文名的条目手填一个 ASCII 显示名（工卡只有拉丁字体）。
   工卡面板里每行还能单独改**副标题**（卡上名字下面那行小字，缺省是账号）：
   清空就是不显示，此时名字会在那一行居中。
...
同步页始终全亮。空闲待机实测约 **13.5 mA**，520 mAh 电池外推约 **38 小时**；
省电措施、实测方法与踩过的坑见 [`docs/power.zh_CN.md`](docs/power.zh_CN.md)。

列表页顶栏：**左上角是本地时间 `HH:MM`**（时区由网页在同步时下发），**右上角是电量百分比**
（低于 20% 转红；电量计读不到时显示 `--`）。倒计时条在最后 5 秒变红。

每一行的样子和网页上的那张卡片一致：左边一块斜着的品牌图标、整行铺着一层很淡的品牌色，
右边是名字 + 账号与大号验证码。工卡上没有矢量渲染器，**图标是网页在推送时画好、
连同令牌一起推上去的**（48×48 的小位图，格式见
[`docs/protocol.zh_CN.md`](docs/protocol.zh_CN.md) 的「图标位图」）——因此带图标的一次推送
要比只推令牌慢十几秒。认不出发行方的条目，网页会画一块首字母色块，工卡照样显示。
